...
namespace deserialization
{
    ...
    void deserializeFieldToSignedInt(BT::ActionNodeBase& _node, const std::string& _port, const Json& _field)
    {
    ...
    void deserializeFieldToUnsignedInt(BT::ActionNodeBase& _node, const std::string& _port, const Json& _field)
    {
        const auto port_info = _node.getPortInfo(_port);
        ...
        if(port_info.has_value() && !port_info.value().missingTypeInfo())
        {
            ...
            if(*type_info == typeid(unsigned short)) // shall match uint16_t
                _node.setOutput<unsigned short>(_port, _field.get<unsigned short>());
            else if(*type_info == typeid(unsigned int)) // shall match uint32_t
                _node.setOutput<unsigned int>(_port, _field.get<unsigned int>());
            else if(*type_info == typeid(unsigned long)) // shall match uint64_t and size_t
                _node.setOutput<unsigned long>(_port, _field.get<unsigned long>());
             else 
    ...
    static const Utils::UnorderedMap<Json::value_t, DeserializeFieldFunction> deserialize_field_map
    {
        ...
        { Json::value_t::number_integer,  [] (auto& _node, const auto& _port, const auto& _field) { deserializeFieldToSignedInt(_node, _port, _field);     }},
        { Json::value_t::number_unsigned, [] (auto& _node, const auto& _port, const auto& _field) { deserializeFieldToUnsignedInt(_node, _port, _field);    }},
