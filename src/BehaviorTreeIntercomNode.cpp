...
namespace BT_ROS
{
    ...
    RosHandShakeData::RosHandShakeData() :
        handshakedata_action_server_ (public_node_handle_, "behavior_tree/handshakedata", boost::bind(&RosHandShakeData::ThreeWayHandShakeDataActionCallback, this, _1), false)
    {

        // Get intercom topic names
        std::string handshake_server_topic_name = private_node_handle_.param<std::string>("handshakedata_server_topic_name", "/server/bt_handshakedata");  
        std::string handshake_client_topic_name = private_node_handle_.param<std::string>("handshakedata_client_topic_name", "/client/bt_handshakedata"); 
        ...
        {
            // Publisher of updates on server side
            sync_publisher_ = public_node_handle_.advertise<HandShakeData>(handshake_server_topic_name, 1, true);    

            //Subscriber to updates from client side
            sync_subscriber_ =  public_node_handle_.subscribe(handshake_client_topic_name, 10, &RosHandShakeData::ThreeWayHandShakeDataTopicCallbackServer, this);    
        }
        else
        {       
            // Publisher of updates on client side
            sync_publisher_ = public_node_handle_.advertise<HandShakeData>(handshake_client_topic_name, 1, true);    
                                          
            //Subscriber of updates from server side
            sync_subscriber_ =  public_node_handle_.subscribe(handshake_server_topic_name, 10, &RosHandShakeData::ThreeWayHandShakeDataTopicCallbackClient, this);    
        }

        // Actionlib
        handshakedata_action_server_.registerPreemptCallback(boost::bind(&RosHandShakeData::ThreeWayHandShakeDataActionPreemptCallback, this));
        handshakedata_action_server_.start();

        // Publish Timer
        pub_timer_ = public_node_handle_.createTimer(ros::Duration(pub_period_s_), &RosHandShakeData::PublishSmsCallback, this, false, false);
    }    
    
    void RosHandShakeData::PublishSmsCallback(const ros::TimerEvent& ev)
    {
        sync_publisher_.publish(msg_to_send_);
        std::string mode_str = "CLIENT";
        if (handshake_mode_ == "server")
    ...
    void RosHandShakeData::ThreeWayHandShakeDataActionCallback(const PerformHandShakeDataGoalConstPtr& _goal_msg)
    {
        action_cancelled_.store(false);
        my_seq_id_ = _goal_msg->request.seq_id;
        data_to_send = _goal_msg->request.data;
        if (handshake_mode_ == "server"){
            ThreeWayHandshakeDataServer(_goal_msg);
        }else{
            ThreeWayHandshakeDataClient(_goal_msg);
        }

        if(handshakedata_action_server_.isActive())
        {
            ROS_INFO("Handshake succeeded!!"); 

            handshakedata_result_.result = true;
            handshakedata_result_.data = data_rx;
            handshakedata_action_server_.setSucceeded(handshakedata_result_, "Synchronization succeeded!");
        }
    }    

    void RosHandShakeData::ThreeWayHandshakeDataServer(const PerformHandShakeDataGoalConstPtr& _goal_msg){
        
        ROS_INFO("ThreeWayHandshakeData Server START");
    ...
    void RosHandShakeData::ThreeWayHandshakeDataClient(const PerformHandShakeDataGoalConstPtr& _goal_msg){
        
        ROS_INFO("ThreeWayHandshakeData Client START");
    ...
    void RosHandShakeData::ThreeWayHandShakeDataActionPreemptCallback()
    {
        ROS_INFO("Handshake Action Goal canceled!");
        ...
        handshakedata_result_.result = false;
        handshakedata_result_.data = "";
        handshakedata_action_server_.setPreempted(handshakedata_result_, "Goal preempted");
    }    

    void RosHandShakeData::ThreeWayHandShakeDataTopicCallbackClient(const behavior_tree_ros::HandShakeData& _topic_msg)
    {
        if(my_seq_id_ == _topic_msg.seq_id)
        {
            // NORMAL CASE
    ...
    void RosHandShakeData::ThreeWayHandShakeDataTopicCallbackServer(const behavior_tree_ros::HandShakeData& _topic_msg)
    {
